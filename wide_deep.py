...
class WideDeepEstimator(BaseEstimator):
    def __init__(self,wide_hparams,deep_hparams,data_source):
        super(WideDeepEstimator, self).__init__(data_source)
        ...
        self._current_deep_logit=None

    def _predict_proba(self,example_id,wide_logit):
        deep_logit=self._current_deep_logit[example_id]
    ...
    def train_batch(self,features,labels):
        self._current_deep_logit=self._dnn.forward(features)
        pred_probas=self._wide_layer.train(features,labels)
        self._dnn.backward(pred_probas-labels)
        return pred_probas
    ...
    def predict(self,features):
        ...
        wide_logits=self._wide_layer.predict_logit(features)
